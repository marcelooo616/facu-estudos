...
export const CardMateria: React.FC<CardMateriaProps> = ({
  ...
  const [modalUnidadeManualAberto, setModalUnidadeManualAberto] = useState(false);
  ...
  return (
    <div className="sleek-card p-5 space-y-4 animate-sleek-in">
      {/* Cabeçalho */}
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-zinc-400 mt-1 leading-relaxed">{materia.descricao}</p>
        </div>

        <button
          onClick={() => aoExcluirMateria(materia.id)}
          className="p-1.5 text-zinc-500 hover:text-red-400 hover:bg-[#0F172A] rounded-lg transition-colors cursor-pointer"
      ...
      <div className="space-y-2 pt-3 border-t border-[#1E293B]">
        <div className="flex items-center justify-between gap-2 flex-wrap">
          ...
          <div className="flex items-center space-x-1.5">
            <button
              onClick={() => setModalUnidadeManualAberto(true)}
              className="btn-sleek-secondary flex items-center space-x-1 text-xs px-2.5 py-1 rounded-lg cursor-pointer font-mono"
            >
            ...
            <button
              ...
              className="btn-sleek-primary flex items-center space-x-1 text-xs px-2.5 py-1 rounded-lg cursor-pointer"
            >
        ...
        {unidadesDaMateria.length > 0 ? (
          <div className="space-y-1.5 max-h-48 overflow-y-auto pr-1">
            {unidadesDaMateria.map((uni) => (
              <div
                key={uni.id}
                className="flex items-center justify-between bg-[#0F172A] p-2.5 rounded-lg border border-[#1E293B] text-xs text-zinc-300 group hover:border-[#3B82F6] transition-colors"
              >
                <div className="flex items-center space-x-2 truncate">
                  <BookOpen className="w-3.5 h-3.5 text-[#3B82F6] flex-shrink-0" />
                ...
                <div className="flex items-center space-x-3 flex-shrink-0">
                  ...
                  <button
                    ...
                    className="opacity-0 group-hover:opacity-100 text-zinc-500 hover:text-red-400 transition-opacity cursor-pointer"
                  >
      ...
      <ModalNovaUnidade
        estaAberto={modalUnidadeManualAberto}
        materiaNome={materia.nome}
        aoFechar={() => setModalUnidadeManualAberto(false)}
        aoSalvar={(dados) => aoCriarUnidadeManual({ ...dados, materiaId: materia.id })}
      />
    </div>
  );
};
